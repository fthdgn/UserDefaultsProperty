...
class UserDefaultsEncoderData {
    ...
    func encode(key codingKey: [CodingKey], value: Any?) {
        data = updateData(data: data, key: codingKey, value: value)
    }
...
struct _Encoder: Encoder {
    ...
    func container<Key: CodingKey>(keyedBy type: Key.Type) -> KeyedEncodingContainer<Key> {
        return KeyedEncodingContainer(_KeyedEncodingContainerProtocol(to: data, codingPath: codingPath))
    }
    
    func unkeyedContainer() -> UnkeyedEncodingContainer {
        return _UnkeyedEncodingContainer(to: data, codingPath: codingPath)
    }
    
    func singleValueContainer() -> SingleValueEncodingContainer {
        return _SingleValueEncodingContainer(to: data, codingPath: codingPath)
    }
}
